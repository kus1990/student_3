...
public class issue2 {
  ...
  public void testIssue2() throws Exception {
    driver.get(baseUrl + "/");
    try {
      assertEquals("https://www.ulmart.ru/", driver.getCurrentUrl());
    } catch (Error e) {
    ...
    try {
      assertTrue(isElementPresent(By.xpath("//span[text()=\"Горнолыжный спорт\"]/parent::a")));
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    driver.findElement(By.xpath("//span[text()=\"Горнолыжный спорт\"]/parent::a")).click();
    try {
      assertEquals("Горные лыжи", driver.findElement(By.xpath("//h1[text()]")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
    ...
    try {
      assertTrue(isElementPresent(By.xpath("//section[contains(@class,\"b-product\")][1]/div/div/div[@class=\"b-product__descr\"][contains(text(),\"Горные лыжи\")]")));
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
